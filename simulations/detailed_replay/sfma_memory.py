...
class SFMAMemory():
    ...
    def replay(self, replay_length: int, current_state=None, current_action=None) -> list:
        ...
        for step in range(replay_length):
            ...
            if self.R_normalize:
                R /= np.amax(R)
            # store variable trace
            self.var_trace['C'].append(np.copy(C))
            self.var_trace['D'].append(np.copy(D))
            self.var_trace['I'].append(np.copy(I))
            self.var_trace['R'].append(np.copy(R))
            exp = np.argmax(R)
            if not self.deterministic:
                ...
                self.var_trace['P'].append(np.copy(probs))
