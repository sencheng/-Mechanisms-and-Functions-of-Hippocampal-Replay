...
def single_run(replay_type, number_of_replays):
    '''
    ...
    if visual_output:
        main_window = qg.GraphicsWindow(title='Simulation: Learning with Random Sequences')
...
if __name__ == '__main__':
    ...
    for replay_type in replay_types:
        for number_of_replays in numbers_of_replays:
            print('Type: ', replay_type, 'Replays: ', number_of_replays)
            latency = []
            for run in range(number_of_runs):
                print('\tRun: ', run+1)
                latency.append(single_run(replay_type, number_of_replays))
            file_name = cwd + '/data/random_type_' + replay_type + '_replays_' + str(number_of_replays) + '.pkl'
