...
def single_run(world, mode):
    ...
    if visual_output:
        main_window = qg.GraphicsWindow(title='Simulation: Random Q')
    ...
    rl_agent = SFMAAgent(modules['rl_interface'], 0.3, 5, 0.9, 0.9, gamma_SR=0.1,
                         custom_callbacks={'on_trial_end': [escape_latency_monitor.update]})
    rl_agent.Q = np.random.normal(rl_agent.Q, 0.1)
...
if __name__ == '__main__':
    ...
    for mode in modes:
        ...
        fileName = cwd + '/data/randomQ_tmaze_mode_' + mode + '.pkl'
    ...
    for mode in modes:
        ...
        fileName = cwd + '/data/randomQ_box_mode_' + mode + '.pkl'
        pickle.dump(steps, open(fileName, 'wb'))
